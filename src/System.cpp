...
ntask = input->ntask;                             // number of task        
...
moleculeTarget = new MoleculeTarget(targetFilename, gas_buffer_flag, user_ff, user_ff_flag); 
...
double info_start = omp_get_wtime();

// create molecule target array
int natoms = moleculeTarget->natoms;
double *x, *y, *z, *q, *eps, *sig;
...
for (int i = 0; i < natoms; i++) {
  x[i] = moleculeTarget->x[i];
  y[i] = moleculeTarget->y[i];
  z[i] = moleculeTarget->z[i];
  q[i] = moleculeTarget->q[i];
  eps[i] = moleculeTarget->eps[i];
  sig[i] = moleculeTarget->sig[i];
}

// create linked-cell information
int Ncells = linkedcell->Ncells;
int N1, N2, Natoms;
int *n1_cells;
int *n2_cells;
int *atoms_cell;
...
n1_cells = new int [Ncells];
n2_cells = new int [Ncells];
atoms_cell = new int [Ncells];

N1 = 0;
N2 = 0;
Natoms = 0;
for (int i = 0; i < Ncells; i++) {
   n1_cells[i] = linkedcell->neighbors1_cells[i];
   n2_cells[i] = linkedcell->neighbors2_cells[i];
   atoms_cell[i] = linkedcell->atoms_inside_cell[i];
   N1 += n1_cells[i];
   N2 += n2_cells[i];
   Natoms += atoms_cell[i];
}

n1_cells_ids = new int [N1];
n2_cells_ids = new int [N2];
ids= new int [Natoms];
...
for (int i = 0; i < Ncells; i++) {
  if (n1_cells[i]) {
    for (int j = 0; j < n1_cells[i]; j++) {
      n1_cells_ids[id1] = linkedcell->neighbors1_cells_ids[i][j];
      id1 += 1;
    }
  }

  if (n2_cells[i]) {
    for (int j = 0; j < n2_cells[i]; j++) {
      n2_cells_ids[id2] = linkedcell->neighbors2_cells_ids[i][j];
      id2 += 1;
    }
  }
  
  if (atoms_cell[i]) {
    for (int j = 0; j < atoms_cell[i]; j++) {
      ids[id3] = linkedcell->atoms_ids[i][j];
      id3 += 1;
    }
  }
}
...
if (r == 0) {
  for (int i = 0; i < ntask; i++) nlocal[i] = Q;
} else {
...
#pragma omp single
{
    for (int i = 0; i < ntask; i++) {
      #pragma omp target nowait \
       ...
       depend(in: Ncells, N1, N2, Natoms, lj_cutoff, alpha, coul_cutoff, skin, lx, ly, lz, Nx, Ny, Nz, natoms, gas_buffer_flag, dt, bmax, mu, temperatureTarget, a, b, c, force_type)
       {
        printf("[WORKER] = %f\n", lj_cutoff+coul_cutoff);
      }
    }
}*/
/*
    ...
    for (int i = 0; i < ntask; i++) {
      ...
      {  
        ...
        for (int j = 0; j < nlocal[i]; j++) {
          ...
          //if (hit) printf("hit traj: %i\n", k);
          //else printf("not hit traj; %i\n", k);  
          ...
          //printf("domega_vec[%i] = %f  traj: %i\n",k,dOmega_vec[k], k);
          //printf("Nscatter_vec[%i] = %i  traj: %i\n",k,Nscatter_vec[k], k);
          //printf("Nlost_vec[%i] = %i  traj: %i\n",k,Nlost_vec[k], k);     
          //printf("Nfree_vec[%i] = %i  traj: %i\n",k,Nfree_vec[k], k);
          delete gasProbe;
          delete force;    
        }
...
// Helium gas dynamics
void System::run_He(GasProbe *gasProbe, bool &success, double &chi, double time_step, Force *force) {
double dt = time_step;
double Ei, Ef, dE, E;
double Ui, Ki, dK, dU;
double Up, Ek;
...
void System::run_N2(GasProbe *gasProbe, bool &success, double &chi, double time_step, Force *force) {
double dt = time_step;
double Ei, Ef, dE, E;
double Ui, Ki, dK, dU;
...
void System::run_CO2(GasProbe *gasProbe, bool &success, double &chi, double time_step, Force *force) {
double dt = time_step;
 ...
 */
void System::setup(GasProbe *gasProbe, bool &hit, double rndVal1, double rndVal2, double rndVal3, double rndVal4, 
...
//printf("cm gas: %f %f %f %f %f %f\n",gasProbe->rcm[0],gasProbe->rcm[1],gasProbe->rcm[2],gasProbe->vcm[0],gasProbe->vcm[1],gasProbe->vcm[2]);
...
if (gas_buffer_flag == 1) {
  ...
  //printf("omega %g %f %f %f %f\n", BOLTZMANN_K, OMEGA_TO_FS_INV, temperatureTarget, Inertia, rndVal9);
  ...
  for (int i = 0; i < natoms; i++) {
    ...
    gasProbe->vy[i] += vcm[1];
    gasProbe->vz[i] += vcm[2];
    //printf("gas: %f %f %f %f %f %f\n",gasProbe->x[i],gasProbe->y[i],gasProbe->z[i],gasProbe->vx[i],gasProbe->vy[i],gasProbe->vz[i]);
  }  
